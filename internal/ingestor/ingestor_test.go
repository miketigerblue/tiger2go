...
func TestFetchAndSave_Integration(t *testing.T) {
	...
	_, _ = pool.Exec(ctx, "DELETE FROM archive WHERE feed_url = $1", mockServer.URL)
	_, _ = pool.Exec(ctx, "DELETE FROM current WHERE feed_url = $1", mockServer.URL)
	...
	err = pool.QueryRow(ctx, "SELECT count(*) FROM archive WHERE feed_url = $1", mockServer.URL).Scan(&archiveCount)
	require.NoError(t, err)
	assert.Equal(t, 2, archiveCount, "expected 2 items in archive")

	err = pool.QueryRow(ctx, "SELECT count(*) FROM current WHERE feed_url = $1", mockServer.URL).Scan(&currentCount)
	require.NoError(t, err)
	assert.Equal(t, 2, currentCount, "expected 2 items in current")

	// Verify content was stored
	var title, summary string
	err = pool.QueryRow(ctx, "SELECT title, summary FROM archive WHERE guid = 'test-guid-001' AND feed_url = $1", mockServer.URL).Scan(&title, &summary)
	require.NoError(t, err)
...
func TestFetchAndSave_EmptyContent(t *testing.T) {
	...
	mockServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/rss+xml")
		fmt.Fprint(w, testRSSEmptyContent)
	}))
	defer mockServer.Close()
	...
	var count int
	err = pool.QueryRow(ctx, "SELECT count(*) FROM archive WHERE feed_url = $1", mockServer.URL).Scan(&count)
	require.NoError(t, err)
...
func TestFetchAndSave_XSSSanitization(t *testing.T) {
	...
	var summary string
	err = pool.QueryRow(ctx, "SELECT summary FROM archive WHERE guid = 'test-guid-xss' AND feed_url = $1", mockServer.URL).Scan(&summary)
	require.NoError(t, err)
...
func TestFetchAndSave_HTTPError(t *testing.T) {
	...
	// This test doesn't need a DB — it should fail at the HTTP fetch stage
	// We need a pool to construct the client, but it won't be used
	databaseURL, ok := os.LookupEnv("DATABASE_URL")
	if !ok || databaseURL == "" {
		t.Skip("DATABASE_URL not set; skipping integration test")
	}
